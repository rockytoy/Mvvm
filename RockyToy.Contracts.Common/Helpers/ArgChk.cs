...
namespace RockyToy.Contracts.Common.Helpers
{
	...
	public static class ArgChk
	{
		...
		/// <summary>
		///   assert(<paramref name="argument" /> &lt; <paramref name="lengthOffset" /> &amp;&amp; <paramref name="argument" />
		...
		[DebuggerStepThrough]
		[LocalizationRequired(false)]
		public static void InBound<T>([NotNull] T argument, [InvokerParameterName] string argumentName, T lengthOffset,
			T offset = default(T))
			where T : IComparable<T>
		{
			...
			if (argument.CompareTo(offset) < 0 || argument.CompareTo(lengthOffset) >= 0)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"{argumentName} ({argument}) is out of bound [{offset},{lengthOffset})");
		}

		/// <summary>
		///   assert(<paramref name="argument" /> &lt; <paramref name="collection" />.Count &amp;&amp; <paramref name="argument" />
		///   &gt;= 0)
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <param name="argument"></param>
		...
		[DebuggerStepThrough]
		[LocalizationRequired(false)]
		public static void InBound<T>(int argument, [InvokerParameterName] string argumentName,
			...
			where T : IComparable<T>
		{
			...
			if (argument.CompareTo(value) <= 0)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"{argumentName} ({argument}) is not greater than {value})");
		}
			...
			where T : IComparable<T>
		{
			...
			if (argument.CompareTo(value) >= 0)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"{argumentName} ({argument}) is not less than {value})");
		}
			...
			where T : IComparable<T>
		{
			...
			if (argument.CompareTo(value) != 0)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"{argumentName} ({argument}) is not equal to {value})");
		}
		...
		public static void Lt<T>([NotNull] ICollection<T> argument, [InvokerParameterName] string argumentName, int size)
		{
			...
			if (argument.Count >= size)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"size of {argumentName} ({argument.Count}) is not less than {size})");
		}
		...
		public static void Gt<T>([NotNull] ICollection<T> argument, [InvokerParameterName] string argumentName, int size)
		{
			...
			if (argument.Count <= size)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"size of {argumentName} ({argument.Count}) is not greater than {size})");
		}
		...
		public static void Eq<T>([NotNull] ICollection<T> argument, [InvokerParameterName] string argumentName, int size)
		{
			...
			if (argument.Count != size)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"size of {argumentName} ({argument.Count}) is not equal to {size})");
		}
		...
		public static void Ne<T>([NotNull] ICollection<T> argument, [InvokerParameterName] string argumentName, int size)
		{
			...
			if (argument.Count == size)
				throw new ArgumentOutOfRangeException(argumentName, argument,
					$"size of {argumentName} ({argument.Count}) is equal to {size})");
		}
